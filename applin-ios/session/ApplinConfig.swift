...
let APPLIN_SERVER_ERROR_PAGE_KEY = "/applin-rpc-error-modal"
...
/// Include an ErrorDetails widget to display the message.
let APPLIN_CLIENT_ERROR_PAGE_KEY = "/applin-app-error"
let APPLIN_ERROR_DETAILS_PAGE_KEY = "/error-details"
let APPLIN_STATUS_PAGE_KEY = "/status"
let APPLIN_SUPPORT_PAGE_KEY = "/support"

class ApplinConfig {
    static func defaultNetworkErrorPage(_ config: ApplinConfig) -> PageSpec {
        ModalSpec(
                ...
                [
                    ModalButtonSpec(text: "Error Details", [.push(APPLIN_ERROR_DETAILS_PAGE_KEY)]),
                    ModalButtonSpec(text: "Server Status", [.push(APPLIN_STATUS_PAGE_KEY)]),
                    ModalButtonSpec(text: "Support", [.push(APPLIN_SUPPORT_PAGE_KEY)]),
    ...
    static func defaultServerErrorPage(_ config: ApplinConfig) -> PageSpec {
        ModalSpec(
                pageKey: APPLIN_SERVER_ERROR_PAGE_KEY,
                kind: .alert,
    ...
    static func defaultClientErrorPage(_ config: ApplinConfig) -> PageSpec {
        ModalSpec(
                pageKey: APPLIN_CLIENT_ERROR_PAGE_KEY,
                kind: .alert,
                // TODO: Include error code in title.
                title: "Error",
    ...
    static func defaultErrorDetailsPage(_ config: ApplinConfig) -> PageSpec {
        NavPageSpec(pageKey: APPLIN_ERROR_DETAILS_PAGE_KEY, title: "Error Details", ScrollSpec(
                LastErrorTextSpec()
        )).toSpec()
    }

    static func defaultStatusPage(_ config: ApplinConfig) -> PageSpec {
        NavPageSpec(pageKey: APPLIN_STATUS_PAGE_KEY, title: "Server Status", ScrollSpec(
    ...
    static func defaultPageNotFoundPage(_ config: ApplinConfig) -> PageSpec {
        NavPageSpec(
    ...
    static func defaultUserErrorPage(_ config: ApplinConfig) -> PageSpec {
        ModalSpec(
    ...
    static func defaultLegalFormPage(_ config: ApplinConfig) -> PageSpec {
    ...
    static func defaultTermsPage(_ config: ApplinConfig) -> PageSpec {
        NavPageSpec(pageKey: "/terms", title: "Terms of Use", ScrollSpec(TextSpec(
                """
                TERMS OF USE
                ...
                Any DMCA Takedown Notices should be sent to: \(ApplinCustomConfig.SUPPORT_EMAIL_ADDRESS)
    ...
    static func defaultPrivacyPolicyPage(_ config: ApplinConfig) -> PageSpec {
        NavPageSpec(pageKey: "/privacy", title: "Privacy Policy", ScrollSpec(TextSpec(
                """
                PRIVACY POLICY
                ...
                We do not target or allow persons under 18 years of age to use the Service, and we do not knowingly collect information from persons under the age of 16. If you are a parent or legal guardian who discovers that your child has provided us with information without your consent, you may contact us at \(ApplinCustomConfig.SUPPORT_EMAIL_ADDRESS), and we will promptly delete such information from our files.
                ...
                If you have any questions about our privacy practices, this Privacy Policy, or how to lodge a complaint with the appropriate authority, please contact us by email at \(ApplinCustomConfig.SUPPORT_EMAIL_ADDRESS).
                """
        ))).toSpec()
    }
