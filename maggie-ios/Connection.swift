    ...
    func connectOnce(_ session: MaggieSession) async throws {
        ...
        if httpResponse.statusCode != 200 {
            ...
            if httpResponse.contentTypeBase() == "text/plain" {
                ...
                print("MaggieConnection server error: "
        ...
        if httpResponse.contentTypeBase() != "text/event-stream" {
            self.state = .serverError
            print("MaggieConnection server sent unexpected content-type: \" \(httpResponse.mimeType ?? "")\"")
            return
        }
        ...
        print("MaggieConnection reading")
        for try await line in asyncBytes.lines {
            if Task.isCancelled {
                return
            }
            // https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events
            print("MaggieConnection read \(line)")
            let parts = line.split(separator: ":", maxSplits: 1)
            if parts.count != 2 || parts[1].isEmpty {
                throw MaggieError.networkError("server sent line with unexpected format: \"\(line)\"")
            }
            if parts[0] != "data" {
                print("MaggieConnection ignoring non-data line from server: \"\(line)\"")
                continue
            }
            let data = parts[1].data(using: .utf8)!
            _ = session.applyUpdate(data)
        }
        print("MaggieConnection disconnected")
    }

    public func start(_ session: MaggieSession) {
        if self.task != nil {
            return
        }
        self.task = Task(priority: .medium) {
            print("MaggieConnection start")
            while self.running {
            ...
            while !Task.isCancelled {
                do {
                    ...
                    self.state = .connectError
                    print("MaggieConnection error: \(error)")
                }
                await sleep(ms: 5_000)
            }
            print("MaggieConnection stop")
        }
    }
